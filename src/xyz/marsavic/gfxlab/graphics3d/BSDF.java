...
public interface BSDF {
	...
	static BSDF avg(BSDF[] bsdfs, double[] weights) {
	...
	BSDF REFLECTIVE = (sampler, n_, i) -> new Result(GeometryUtils.reflected(n_, i), Color.WHITE);
	...
	static BSDF diffuse(Color c) {
		return (sampler, n_, i) -> new Result(GeometryUtils.sampleHemisphereCosineDistributedRejectionN(sampler, n_), c);
		
//		Alternative, using uniform sampling
//		return (sampler, n_, i) -> {
//			Vec3 out = GeometryUtils.sampleHemisphereUniform(sampler, n_);
...
/*
	...
	static BSDF glossy(Color c, double s) {
		return (sampler, n_, i) -> {
			Vec3 d = GeometryUtils.sampleHemisphereCosineDistributedRejectionN(sampler, n_);
			Vec3 r = GeometryUtils.reflected(n_, i.normalized_());
...
/*
	...
	static BSDF glossy(Color c, double s) {
		return (sampler, n_, i) -> {
			Vec3 r = i;
			while (true) {
				Vec3 u = GeometryUtils.sampleHemisphereCosineDistributedRejectionN(sampler, n_);
				Vec3 n = Vec3.lerp(n_, u, s);
				r = GeometryUtils.reflected(n, r);
				if (r.dot(n_) > 0) {
...
*/
	...
	static BSDF glossyRefractive(Color c, double refractiveIndex, double s) {
		return (sampler, n_, i) -> {
			Vec3 r = GeometryUtils.refractedNN(n_, i.normalized_(), refractiveIndex); // TODO Maybe use GeometryUtils.refractedN, without normalizing, to avoid one sqrt.
	...
	static BSDF refractive(Color c, double refractiveIndex) {
		...
		return (sampler, n_, i) -> new Result(GeometryUtils.refractedNN(n_, i.normalized_(), refractiveIndex), c);  // TODO Maybe use GeometryUtils.refractedN, without normalizing, to avoid one sqrt.
	}
